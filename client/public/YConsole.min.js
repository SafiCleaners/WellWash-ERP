(function(){var f=document.createElement("style");f.appendChild(document.createTextNode('\n.YConsole_left_pan{position:fixed;\ntransition:margin 0.5s ease;z-index:1000000000;}\n.YConsole_left_pan_switch{position:fixed;\ntext-align:left;color:#FFF;font-family:Arial;font-size:18px;\ntext-shadow:-2px 0px 2px #FF0000,-1px 0px 2px #C04080,0px 0px 2px #8080FF,\n1px 0px 2px #40C0FF,2px 0px 2px #00FFFF;opacity:0.5;\ncursor:pointer;z-index:1000000001;}\n.YConsole_left_pan_resize{position:fixed;text-align:center;color:#FFF;background-color:#000;}\n.YConsole_left_pan__dock_left{left:0px;top:0px;}\n.YConsole_left_pan__dock_right{right:0px;top:0px;}\n.YConsole_left_pan__dock_top{left:0px;top:0px;}\n.YConsole_left_pan__dock_bottom{left:0px;bottom:0px;}\n.YConsole_left_pan_switch__dock_left{left:0px;top:0px;}\n.YConsole_left_pan_switch__dock_right{right:0px;top:0px;}\n.YConsole_left_pan_switch__dock_top{right:20px;top:5px;}\n.YConsole_left_pan_switch__dock_bottom{right:20px;bottom:5px;}\n.YConsole_left_pan_resize__dock_left{cursor:ew-resize;}\n.YConsole_left_pan_resize__dock_right{cursor:ew-resize;}\n.YConsole_left_pan_resize__dock_top{cursor:ns-resize;}\n.YConsole_left_pan_resize__dock_bottom{cursor:ns-resize;}\n.YConsole_konsol_selector{display:inline-block;}\n.YConsole_konsol_selector_value{display:inline-block;cursor:pointer;background-color:#444;margin:0px 3px;padding:1px 3px;border-radius:10px;}\n.YConsole_konsol_selector_value:hover{background-color:#666;}\n.YConsole_konsol_selector_childs{display:block;position:absolute;border:solid 1px #000;background-color:#eee;box-shadow:3px 2px 5px #888;}\n.YConsole_konsol_selector_option{display:block;color:#000;background-color:#fff;padding:0px 2px;margin:2px 2px;cursor:pointer;}\n.YConsole_konsol_selector_option:hover{color:#fff;background-color:#000;}\n.YConsole_konsol_help{position:absolute;left:0px;top:0px;background-color:#FFF;width:100%;overflow:auto;font-family:Arial, Helvetica, sans-serif;font-size:16px;}\n.YConsole_konsol_help div{margin:3px;padding:3px;}\n.YConsole_konsol_help div div{margin:0px 8px;padding:6px 8px;border:solid 1px #CCC;}\n.YConsole_konsol_help div div div{padding:3px 8px;border:none;background-color:#eee;}\n.YConsole_konsol_help_exit{position:absolute;right:20px;top:10px;padding:0px 6px;font-family:"Arial Black", Gadget, sans-serif;font-size:24px;color:#FFF;background-color:rgba(0,0,255,0.5);border-radius:4px;cursor:pointer;}\n.YConsole_konsol{border:solid 1px #000;font-family:Arial;}\n.YConsole_konsol_tools{color:#fff;background-color:#666;padding-bottom:1px;}\n.YConsole_konsol_tools img {vertical-align: middle;}\n.YConsole_konsol_tools a{display:inline-block;text-align:center;\nbackground-color:#777;cursor:pointer;}\n.YConsole_konsol_help .YConsole_konsol_tools a{padding:0px 3px;}\n.YConsole_konsol_tools a:hover{background-color:transparent;}\n.YConsole_konsol_tools span span{cursor:pointer;}\n.YConsole_konsol_list{background-color:#AAA;\noverflow:auto;user-select: initial;}\n.YConsole_konsol_split{background-color:#444;cursor:ns-resize;}\n.YConsole_konsol_counter{display:inline-block;padding:0px 5px;color:#FFF;background-color:#063;border-radius:10px;margin-left:5px;}\n.YConsole_konsol_line{font-family:monospace;font-size:14px;border-bottom:solid 1px #000;}\n.YConsole_konsol_showstack{}\n.YConsole_konsol_msg{background-color:#fff;}\n.YConsole_konsol_msg_u_undefined{}\n.YConsole_konsol_msg_u_number{color:#F00;}\n.YConsole_konsol_msg_u_string{color:#B97800;}\n.YConsole_konsol_msg_u_object{color:#00C;cursor:pointer;}\n.YConsole_konsol_msg_u_array{color:#060;cursor:pointer;}\n.YConsole_konsol_error{background-color:#fee;}\n.YConsole_konsol_stack{background-color:#ddd;}\n.YConsole_konsol_objedit{background-color:#aaa;font-family:monospace;font-size:12px;height:100px;overflow-y:auto;}\n.YConsole_konsol_oe_line{background-color:#fff;border-bottom:solid 1px #999;}\n.YConsole_konsol_oe_label,.YConsole_konsol_oe_type,.YConsole_konsol_oe_val{display:inline-block;}\n.YConsole_konsol_oe_toggler{cursor:pointer;}\n.YConsole_konsol_oe_hide{cursor:pointer;position:absolute;margin-top:3px;right:20px;color:#FFF;background-color:#900;padding:0px 3px;border-radius:20px;border:solid 1px #000000;}\n.YConsole_konsol_oe_hide:hover{background-color:#F00;}\n.YConsole_konsol_oe_label{padding:0px 3px;}\n.YConsole_konsol_oe_type{padding:0px 3px;background-color:#ddd;}\n.YConsole_konsol_oe_val{}\n.YConsole_konsol_oe_childs{margin-left:12px;}\n.YConsole_konsol_msg_switch{display:inline-block;padding:0px 8px;border-radius:20px;background-color:#888;color:#fff;border:solid 1px #fff;cursor:pointer;}\n'));
document.documentElement.appendChild(f)})();
var YConsole=new function(){var f={star:"data:image/png;base64,",autoscroll_off:"data:image/png;base64,",
autoscroll_on:"data:image/png;base64,",
hang_b:"data:image/png;base64,",
hang_l:"data:image/png;base64,",
hang_r:"data:image/png;base64,",
hang_t:"data:image/png;base64,",
save:"data:image/png;base64,",
help:"data:image/png;base64,",
clear:"data:image/png;base64,",
stack_off:"data:image/png;base64,",stack_on:"data:image/png;base64,"},
o=this;this.MAX_OBJ_ARRAY_LENGTH=this.MAX_OBJ_STR_LENGTH=20;this.MAX_LOG_STR_LENGTH=200;this.MAX_LOG_ARRAY_LENGTH=40;var v=5,y=20,q=function(a){var c=document.createElement("div");c.appendChild(document.createTextNode(a));return c.innerHTML.split("\n").join("<br/>")},r=function(a,c){c.objedit&&c.objedit.show(a)},A=function(a){a=window.getComputedStyle(a,null);return[parseInt(a.getPropertyValue("width")),parseInt(a.getPropertyValue("height"))]},z=function(a){if(0==document.styleSheets.length){var c=
document.createElement("style");c.appendChild(document.createTextNode(""));document.body.appendChild(c)}c=document.styleSheets[document.styleSheets.length-1];c.insertRule(a+"{\n\n}",c.cssRules.length);return c.cssRules[c.cssRules.length-1]},e=function(a,c,b){a=document.createElement(a);if("object"==typeof c)for(var d in c)if("style"==d)for(var m in c.style)a.style[m]=c.style[m];else"innerHTML"==d?a.innerHTML=c.innerHTML:a[d]=c[d];if(b)for(d in b)a.appendChild(b[d]);return a},j=function(a,c){"undefined"!=
typeof MSApp?MSApp.execUnsafeLocalFunction(function(){a.appendChild(c)}):a.appendChild(c)},s=function(a,c){"undefined"!=typeof MSApp?MSApp.execUnsafeLocalFunction(function(){a.innerHTML=c}):a.innerHTML=c},C=function(a,c){var b=!1,d=[0,0];"undefined"!=typeof document.ontouchstart?(a.addEventListener("touchstart",function(a){b=!0;d=[a.changedTouches[0].pageX,a.changedTouches[0].pageY]}),window.addEventListener("touchend",function(){b=!1}),window.addEventListener("touchmove",function(a){b&&(a=[a.changedTouches[0].pageX,
a.changedTouches[0].pageY],c([a[0]-d[0],a[1]-d[1]]),d=a)})):(a.addEventListener("mousedown",function(a){b=!0;d=[a.pageX,a.pageY]}),window.addEventListener("mouseup",function(){b=!1}),window.addEventListener("mousemove",function(a){b&&(a=[a.pageX,a.pageY],c([a[0]-d[0],a[1]-d[1]]),d=a)}))},K=function(a){"complete"==document.readyState?a():window.addEventListener("load",a)},D=function(a,c,b){var d=a instanceof Array?"array":typeof a,b=b+"_msg_u_"+d;if("number"!=d){if("string"==d){var d=a+"",m=function(){},
i=c.objedit?o.MAX_LOG_STR_LENGTH:o.MAX_OBJ_STR_LENGTH;d.length>i&&(d=d.substring(0,i)+"\u2026",m=function(){r(a,c)});return e("span",{innerHTML:'"'+q(d)+'"',className:b,onmouseup:m})}if("function"==d)return e("span",{innerHTML:"function",className:b,onmouseup:function(){r(a,c)}});if("array"==d)return d=" "+a.join(" , ")+" ",i=c.objedit?o.MAX_LOG_ARRAY_LENGTH:o.MAX_OBJ_ARRAY_LENGTH,d.length>i&&(d=d.substring(0,i)+" , \u2026"),e("span",{innerHTML:"[ "+d+" ]",className:b,onmouseup:function(){r(a,c)}});
if("object"==d)return e("span",{innerHTML:"Object",className:b,onmouseup:function(){r(a,c)}})}return e("span",{innerHTML:a+"",className:b})},E=z(".YConsole_konsol_tools_button,.YConsole_konsol_tools_button img"),F=z(".YConsole_konsol_resize_vertical"),G=z(".YConsole_konsol_resize_horizontal"),H=function(a,c){var b=[-1,-1],d=[-1,-1];this.force=function(){c(b)};var e=function(){if(a.parentNode){var i=A(a.parentNode);(b[0]!=i[0]||b[1]!=i[1]||b[0]!=d[0]||b[1]!=d[1])&&c(i);d=b;b=i}requestAnimationFrame(e)};
requestAnimationFrame(e)},L=function(a,c){var b=!1,d=e("div",{className:"YConsole_konsol_selector_value"}),m=e("div",{className:"YConsole_konsol_selector_childs",style:{display:"none"}});this.dom=e("div",{className:"YConsole_konsol_selector"},[d,m]);var i=function(b){b=a[b];b instanceof HTMLElement?d.appendChild(b):s(d,""+b)},f=function(b){0==d.innerHTML.length&&i(b);var f=a[b],h=e("span",{className:"YConsole_konsol_selector_option"},[]);f instanceof HTMLElement?h.appendChild(f):s(h,""+f);m.appendChild(h);
h.onmouseup=function(){c(b);i(b)}},g;for(g in a)f(g);d.addEventListener("mouseup",function(){b||requestAnimationFrame(function(){b=!0;m.style.display=""})});document.addEventListener("mouseup",function(){b&&(b=!1,m.style.display="none")})},M=function(a){var c=this,b=e("div",{className:"YConsole_konsol_oe_hide",innerHTML:"X"},[]);b.onmouseup=function(){c.hide()};c.dom=e("div",{className:"YConsole_konsol_objedit",style:{display:"none"}},[b]);c.visible=!1;var d=function(a,b){var c=this,f=!1,g=!1,l=b instanceof
Array?"array":typeof b,j=e("span",{className:"YConsole_konsol_msg_switch",innerHTML:"\u25ba"},[]),k=e("span",{className:"YConsole_konsol_oe_label",innerHTML:a+""},[]),n=e("span",{className:"YConsole_konsol_oe_type",innerHTML:""+l+""},[]),o=e("span",{className:"YConsole_konsol_oe_val"},[D(b,c,"YConsole_konsol")]),t=e("div",{className:"YConsole_konsol_oe_childs",style:{display:"none"}},[]);this.dom=e("div",{className:"YConsole_konsol_oe_node"},[e("div",{className:"YConsole_konsol_oe_line"},[j,k,n,o]),
t]);({"function":1,string:1,object:1,array:1})[l]?j.onmouseup=function(){c.toggle()}:(j.style.visibility="hidden",j.style.cursor="default");this.toggle=function(){if(!g){if(typeof b=="function")s(t,q(b+""));else if(typeof b=="string")s(t,q(b+""));else for(var a in b)t.appendChild((new d(""+a,b[a])).dom);g=true}f=!f;s(j,f?"\u25bc":"\u25ba");t.style.display=f?"":"none"}};this.show=function(e){c.dom.style.display="";c.dom.innerHTML="";c.dom.appendChild(b);c.dom.appendChild((new d("_",e)).dom);c.visible=
!0;a.forceresize()};this.hide=function(){c.dom.style.display="none";c.visible=!1;a.forceresize()}},N=function(){var a=this;this.dom=null;var c=e("span",{className:"YConsole_konsol_help_exit",innerHTML:"X"},[]),b=function(a,b){return"<u>"+a+"</u><div>"+(b.join?b.join("<br/>"):b)+"</div>"},b=["<h2>YConsole Quick Reference</h2>"+b("<b>Misc</b>",["<span class='YConsole_left_pan_switch' style='position:relative;'><img width='24' src=\""+f.star+'"/></span> :',""+b("switcher","hide or show the console pannel.")]),
"",""+b("<b>Tools Pannel</b>",["<span class='YConsole_konsol_tools'><a><img width='24' src=\""+f.autoscroll_on+"\"/></a></span> / <span class='YConsole_konsol_tools'><a><img width='24' src=\""+f.autoscroll_off+'"/></a></span> : ',b("Toggle autoscroll","Enables / disables auto scrolling to last log."),"<span class='YConsole_konsol_tools'><a><img width='20' src=\""+f.save+'"/></a></span> : ',b("Save logs","save logs to a text file."),"<span class='YConsole_konsol_selector_value' style='color:#fff;'><img width='24' src=\""+
f.hang_l+'"/></span> : ',b("Docking",["choose where the console is hanging.","availables are :"," <img width='24' src=\""+f.hang_l+'"/> : left'," <img width='24' src=\""+f.hang_r+'"/> : right'," <img width='24' src=\""+f.hang_t+'"/> : top'," <img width='24' src=\""+f.hang_b+'"/> : bottom']),function(a,b,d){for(var c=[],e=0;e<d;e++){var f=Math.round(a+e/d*(b-a));c.push("<span style='background-color:rgb("+f+","+f+","+f+");'>&nbsp;</span>")}return"<span style='border:solid 1px #888;'>"+c.join("")+"</span>"}(155,
255,8)+" : ",b("Alpha",["modify opacity to view behind the logs."]),"<span class='YConsole_konsol_tools'><a><img width='20' src=\""+f.clear+'"/></a></span> : ',b("clear","clear the log pile.")]),"",""+b("<b>Objects explorer</b>",["You can explore objects, arrays or long strings from logs.","Clicking on it will open the Objects explorer pannel(if not visible).","<span class='YConsole_konsol_oe_hide' style='position:relative;right:auto;'>X</span> :",""+b("Exit","close the Objects explorer pannel.")]),
"",""+b("<b>Credits</b>",["<b>YConsole.js</b> has been made to make js debugging easier under mobile with the lowest impact (no dependencies), exept for '<b>.YConsole_</b>' as a reserved css class name prefix and of course '<b>YConsole</b>' as a reserved js var.","<a href='http://www.yorgsite.fr/experiments/YConsole/doc/' target='_blank'>js documentation</a>.","<a href='http://www.yorgsite.fr' target='_blank'>www.yorgsite.fr</a>."]),""].join("<br/>");this.show=function(){a.dom.style.display="";c.style.display=
""};var d=function(){a.dom.style.display="none";c.style.display="none"};a.dom=e("div",{className:"YConsole_konsol_help"},[e("div",{innerHTML:b}),c]);new H(a.dom,function(b){a.dom.style.height=b[1]+"px";a.dom.parentNode&&a.dom.parentNode!=c.parentNode&&a.dom.parentNode.appendChild(c)});d();c.onmouseup=d},O=function(){var a=this,c,b,d,g;this.objedit=this.dom=null;var i,h=[],k=!0,n,r,p,q=null,w=function(){for(var a=[],b=0,d;d=h[b];b++)a.push(d.toText());return a.join("\r\n----------------------------------------\r\n")},
x=function(b,d,f){this.type=b;this.msg=d;this.trace=f;var g=e("span",{className:"YConsole_konsol_counter",innerHTML:"1"}),h=e("span",{className:"YConsole_konsol_msg_switch",innerHTML:"\u25ba"},[]),i=e("span",{className:"YConsole_konsol_msg_msg"},[]),k=e("span",{style:{"float":"right",color:"#000"},innerHTML:""+f.source}),b=e("div",{className:"YConsole_konsol_"+b+"",innerHTML:""},[h,k,i]),j=e("div",{className:"YConsole_konsol_stack "+c+"_stk",innerHTML:""+f.html});this.dom=e("div",{className:"YConsole_konsol_line"},
[b,j]);var m=!1;h.addEventListener("click",function(){m=!m;h.innerHTML=m?"\u25bc":"\u25ba";j.style.display=m?"block":"none"});if("object"==typeof d){if(i.innerHTML="","number"==typeof d.length)for(f=0;f<d.length;f++)0<f&&i.appendChild(e("span",{innerHTML:" , "})),i.appendChild(D(d[f],a,"YConsole_konsol"))}else i.innerHTML=""+d;i.appendChild(g);g.style.display="none";this.toText=function(){return"["+g.innerHTML+"] "+("object"==typeof d?Array.prototype.concat.apply([],d).join(" , "):i.textContent+"").split("\n").join("\r\n")};
this.cntIncr=function(){g.style.display="";s(g,1+parseInt(g.innerHTML))}},t=function(a,b,d){if(0==h.length||h[h.length-1].type+""!=a+""||h[h.length-1].trace+""!=d+"")return!1;if("object"==typeof b||"array"==typeof b){if(typeof b!=typeof h[h.length-1].msg)return!1;for(var c in b)if(b[c]!=h[h.length-1].msg[c])return!1;for(c in h[h.length-1].msg)if(b[c]!=h[h.length-1].msg[c])return!1}else return h[h.length-1].msg+""==b+"";return!0},u=function(a,b,d){"error"==a&&(b=(""+b).split("Error").join("<b style='color:#f00;'>Error</b>"));
t(a,b,d)?h[h.length-1].cntIncr():(a=new x(a,b,d),h.push(a),g.appendChild(a.dom),k&&a.dom.scrollIntoView())};this.forceresize=function(){q.force()};this.trace_track=function(){var a,b,d;try{eval("5=5;")}catch(c){if("string"==typeof c.stack){for(a=c.stack.split("\n");0<a.length&&-1==a.shift().indexOf("trace_track"););for(;0<a.length&&-1!=a[0].indexOf("YConsole");)a.shift();a=a.join("\n")}else a=""}b=a.split("<").join("&lt;").split(">").join("&gt;").split("\n").join("<br/>");d=a.split("\n")[0].split(")")[0].split("/").pop();
return{stack:a,html:b,source:d}};this.updateDocking=function(){var c;"left"==l?c=[b,a.objedit.dom,d,g]:"right"==l?c=[b,a.objedit.dom,d,g]:"top"==l?c=[a.objedit.dom,d,g,b]:"bottom"==l&&(c=[b,a.objedit.dom,d,g]);a.dom.innerHTML="";for(var e in c)a.dom.appendChild(c[e])};this.clear=function(){h=[];g.innerHTML=""};this.log=function(a,b){u("msg",a,this.trace_track(1+b||0))};this.error=function(b,d){b&&b.error&&b.error.stack?u("error",b.message,b.error.stack.split("\n").join("<br/>")):u("error",b,a.trace_track(1+
d||0))};c="YConsole_konsol_"+Math.round(1E8*Math.random());a.objedit=new M(a);b=e("div",{className:"YConsole_konsol_tools",innerHTML:""});d=e("div",{className:"YConsole_konsol_split",style:{display:"none"}});g=e("div",{className:"YConsole_konsol_list",innerHTML:""});a.dom=e("div",{className:"YConsole_konsol"},[b,a.objedit.dom,d,g]);i=z("."+c+"_stk");E.style.width=y+"px";F.style.height=v+"px";G.style.width=v+"px";a.dom.style.left="0px";a.dom.style.top="0px";q=new H(a.dom,function(c){var e=[b.offsetWidth,
b.offsetHeight],f=[0,0],h=[0,0];a.objedit.visible?(d.style.display="",f=A(a.objedit.dom),h=A(d)):d.style.display="none";g.style.height=c[1]-e[1]-f[1]-h[1]+"px"});requestAnimationFrame(function(){j(b,e("span",{innerHTML:" &nbsp; &nbsp; console &nbsp; "}));n=e("a",{className:"YConsole_konsol_tools_button",innerHTML:'<img src="'+f.autoscroll_on+'"/>',title:"toggle autoscroll[on]",onmouseup:function(){k=!k;s(this,k?'<img src="'+f.autoscroll_on+'"/>':'<img src="'+f.autoscroll_off+'"/>');this.title="toggle autoscroll["+
(k?"on":"off")+"]"}});j(b,n);i.style.display="none";j(b,e("span",{innerHTML:" &nbsp"}));dom_save=e("a",{className:"YConsole_konsol_tools_button",innerHTML:'<img src="'+f.save+'"/>',title:"save console",onmouseup:function(){var a=e("a",{download:"log.txt",href:URL.createObjectURL(new Blob([""+w()]))}),b=document.createEvent("MouseEvent");b.initEvent("click",!0,!0);a.dispatchEvent(b)}},[]);j(b,dom_save);C(d,function(b){a.objedit.dom.style.height=a.objedit.dom.offsetHeight+b[1]+"px";q.force()});var c=
new L({left:"<img width='20' src=\""+f.hang_l+'"/>',right:"<img width='20' src=\""+f.hang_r+'"/>',top:"<img width='20' src=\""+f.hang_t+'"/>',bottom:"<img width='20' src=\""+f.hang_b+'"/>'},function(a){o.docking=a});c.dom.title="docking";j(b,e("span",{innerHTML:" &nbsp"}));j(b,c.dom);p=e("span",{innerHTML:""});for(var c=function(b){var c=Math.round(128*b)+127;p.appendChild(e("span",{innerHTML:"&nbsp;",title:"opacity:"+b,style:{backgroundColor:"rgb("+c+","+c+","+c+")"},onmouseup:function(){a.dom.style.opacity=
b}}))},g=2;11>g;g++)c(g/10);j(b,e("span",{innerHTML:" &nbsp"}));j(b,p);c=e("a",{className:"YConsole_konsol_tools_button",innerHTML:'<img src="'+f.help+'"/>',title:"help",onmouseup:function(){B.show()}});j(b,e("span",{innerHTML:" &nbsp"}));j(b,c);r=e("a",{className:"YConsole_konsol_tools_button",innerHTML:'<img src="'+f.clear+'"/>',title:"clear console",onmouseup:function(){window.confirm("clear console ?")&&a.clear()}});j(b,e("span",{innerHTML:" &nbsp"}));j(b,r);d.style.height=o.SPLITTER_WIDTH+"px"})},
n=null,B=null,I=e("img",{src:f.star}),w=e("div",{className:"YConsole_left_pan_switch"},[I]),k=e("div",{className:"YConsole_left_pan_resize",innerHTML:""},[]),g=e("div",{className:"YConsole_left_pan"},[w,k]),P={left:{margin:"marginLeft",signs:["<img width='20' src=\""+f.hang_l+'"/>',"<img width='20' src=\""+f.hang_r+'"/>']},right:{margin:"marginRight",signs:["<img width='20' src=\""+f.hang_r+'"/>',"<img width='20' src=\""+f.hang_l+'"/>']},top:{margin:"marginTop",signs:["<img width='20' src=\""+f.hang_t+
'"/>',"<img width='20' src=\""+f.hang_b+'"/>']},bottom:{margin:"marginBottom",signs:["<img width='20' src=\""+f.hang_b+'"/>',"<img width='20' src=\""+f.hang_t+'"/>']}},x=[{node:g,defclass:"YConsole_left_pan",styles:{left:{width:"320px",height:"100vh"},right:{width:"320px",height:"100vh"},top:{width:"100vw",height:"320px"},bottom:{width:"100vw",height:"320px"}}},{node:w,defclass:"YConsole_left_pan_switch",styles:{left:{},right:{},top:{},bottom:{}}},{node:k,defclass:"YConsole_left_pan_resize",styles:{left:{left:"320px",
top:"0px",height:"100vh"},right:{right:"320px",top:"0px",height:"100vh"},top:{right:"0px",top:"320px",width:"100vw"},bottom:{right:"0px",bottom:"320px",width:"100vw"}}}],u=1,l="",p=0;w.onmouseup=function(){u=!u;var a=P[l];u?(g.style[a.margin]="0px",k.style.display=""):(g.style[a.margin]="-"+g.offsetWidth+"px",k.style.display="none")};C(k,function(a){"left"==l?(g.style.width=g.offsetWidth+a[0]+"px",k.style.left=parseInt(g.style.width)+"px"):"right"==l?(g.style.width=g.offsetWidth-a[0]+"px",k.style.right=
parseInt(g.style.width)+"px"):"top"==l?(g.style.height=g.offsetHeight+a[1]+"px",k.style.top=parseInt(g.style.height)+"px"):"bottom"==l&&(g.style.height=g.offsetHeight-a[1]+"px",k.style.bottom=parseInt(g.style.height)+"px")});Object.defineProperty(this,"SWITCHER_SIZE",{get:function(){return p},set:function(a){p=Math.max(16,a);I.width=p},enumerable:!0,configurable:!0});Object.defineProperty(this,"TOOLS_SIZE",{get:function(){return y},set:function(a){"number"==typeof a&&(y=Math.max(16,a),E.style.width=
y+"px")},enumerable:!0,configurable:!0});Object.defineProperty(this,"SPLITTER_WIDTH",{get:function(){return v+""},set:function(a){"number"==typeof a&&(v=Math.max(16,a),F.style.height=v+"px",G.style.width=v+"px")},enumerable:!0,configurable:!0});Object.defineProperty(this,"docking",{get:function(){return l+""},set:function(a){var c="left top right bottom width height marginLeft marginRight marginTop marginBottom".split(" ");if(l!=a&&x[0].styles[a]){for(var b=0,d;d=x[b];b++){for(var e=0,f;f=c[e];e++)d.node.style[f]=
"";for(e in d.styles[a])d.node.style[e]=d.styles[a][e];d.node.className=d.defclass+" "+d.defclass+"__dock_"+a}k.className+=" YConsole_konsol_resize_"+("left"==a||"right"==a?"horizontal":"vertical");l=a+"";n&&n.updateDocking();console.log("set console docking to "+l)}},enumerable:!0,configurable:!0});var J=null;this.activate=function(){n||(n=new O,x[0].styles[this.docking]||(this.docking="left"),0==p&&(this.SWITCHER_SIZE=20),J=console.log,console.log=function(){J.apply(console,arguments);n.log(arguments);
window.addEventListener("error",n.error,true)},g.appendChild(n.dom),B=new N,g.appendChild(B.dom))};this.show=function(){this.activate();K(function(){"undefined"!=typeof MSApp?MSApp.execUnsafeLocalFunction(function(){document.body.appendChild(g)}):document.body.appendChild(g)})};this.hide=function(){n&&document.body.removeChild(g)}};
