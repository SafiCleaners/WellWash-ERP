...
const routes = async (app) => {
    ...
    Thank you and see you soon!`,
        ...
        */
        app.use("/billing/confirmation", function (req, res) {
        ...
        app.use("/billing/validation", function (req, res) {
            console.log(JSON.stringify(req.body, null, "\t"));
        ...
        */
        app.use("/billing/lipaCallback/:txid", function (req, res) {
            /*
...
if (FUNCTIONS_FRAMEWORK != true) {
    // If not in GCF environment, start the app listening on a port
    const port = process.env.PORT || 8002;
    return routes(appInitial)
        .then(({ app, client, db }) => {
            app.listen(port, () => {
    ...
    functions.http('helloHttp', async (req, res) => {
        try {
            const { app: appHoisted, client, db } = await routes(appInitial);
            appHoisted(req, res);
        } catch (error) {
            console.error('Error occurred:', error.stack); // log error with stack trace
            res.status(500).json({ error }); // return JSON error response
        }
    });
